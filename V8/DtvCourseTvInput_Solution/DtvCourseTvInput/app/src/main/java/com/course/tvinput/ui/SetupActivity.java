...
public class SetupActivity extends Activity {
    ...
    public void onClickScanAction(View view) {
        ...
        if (view.getId() == mStartScan.getId()) {
            ...
            channelNumber = mDtvEngine.getChannelManager().startScan();

            if(channelNumber != 0) {
                ChannelDescriptor channelDescriptor;
                StringBuilder channelList = new StringBuilder();
                channelList.append("Found " + channelNumber + " channels!");
                channelList.append("\n");
                for (int i = 0; i < channelNumber; i++) {
                    channelDescriptor = mDtvEngine.getChannelManager().getChannelById(i);
                    if(channelDescriptor == null) {
                        continue;
                    }
                    channelList.append(channelDescriptor.getChannelUri());
                    channelList.append("\n");
                }
                mChannelList.setText(channelList.toString());
            } else {
                mChannelList.setText("Failed to scan channels!");
            }
        }
