                                              ...
                                              uint8_t                 *Buffer)
{
    ...
    if(Filter_track == TID_PAT)
    {
        ...
        parser_error = DtvPlayer_ParserPATParse(Buffer, &gPatTable);
        // Check parser status
        ...
        int nit_found = 0;
        int i;
        for(i = 0 ; i < gPatTable.program_size; i++)
        {
            int pmtindex = i - nit_found;
            // Skip NIT Table
            if(gPatTable.programs[i].program_map_pid == 16)
            {
                nit_found = 1;
                continue;
            }
            ...
            gPmtDmxRequest[pmtindex].pid = gPatTable.programs[i].program_map_pid;
            ...
            {
                LOGD("[%s:%d] Created request fo Program map pid: [%d]", __FUNCTION__, __LINE__, gPatTable.programs[i].program_map_pid);
            }  
        }
        ...
        for(int j = 0; j < table.streams_number; j++)
        {
            ...
            // TODO: Fill channel structure
            // NOTE 1: Pay attention to multiple audio tracks
            switch (streamType)
            {
            case STREAM_TYPE_ITU_T_H262_ISO_IEC_13818_2_MPEG_2_VIDEO:
                printf("VPID = %d Codec: MPEG2 CodecID=%d\n", pid, eTDAL_AV_VIDEO_TYPE_MPEG2);
                break;
            case STREAM_TYPE_ISO_IEC_11172_MPEG_1_VIDEO:
                printf("VPID = %d Codec: MPEG1 CodecID=%d\n", pid, eTDAL_AV_VIDEO_TYPE_MPEG1);
                break;
            case STREAM_TYPE_ISO_IEC_14496_10_H264:
                printf("VPID = %d Codec: H264 CodecID=%d\n", pid, eTDAL_AV_VIDEO_TYPE_H264);
                break;
            case STREAM_TYPE_ISO_IEC_11172_MPEG_1_AUDIO:
                printf("APID = %d Codec: MPEG CodecID=%d\n", pid, eTDAL_AV_AUDIO_TYPE_MPEG);
                break;
            case STREAM_TYPE_ISO_IEC_13818_7_AAC:
                printf("APID = %d Codec: AAC CodecID=%d\n", pid, eTDAL_AV_AUDIO_TYPE_MPEG_AAC);
                break;
            default:
                break;
            }
        }
        // TODO: Notify DTV module about channel
    }
}
        ...
        tTDAL_DMX_ChannelId* videoChannelId, tTDAL_DMX_ChannelId* audioChannelId)
{
    ...
    uint32_t filter1;
    dmx_error = TDAL_DMX_Allocate_Filter(*videoChannelId, &filter1);

    dmx_error = TDAL_DMX_Control_Filter(filter1, eTDAL_DMX_CTRL_ENABLE);
    ...
    uint32_t filter2;
    dmx_error = TDAL_DMX_Allocate_Filter(*audioChannelId, &filter2);

    dmx_error = TDAL_DMX_Control_Filter(filter2, eTDAL_DMX_CTRL_ENABLE);
