 ...
 *
 * FILE NAME: $URL: http://svnsrv/svn/iwedia-ext/dev/sources/chal/core/tdal/trunk/inc/tdal_disp.h $
 *            $Rev: 13061 $
 *            $Date: 2014-03-26 11:43:30 +0100 (Wed, 26 Mar 2014) $
 *
...
typedef enum
{
    ...
    eTDAL_DISP_BLENDER_ID_4, /* Blender identifier 4 */
    eTDAL_DISP_BLENDER_ID_5, /* Blender identifier 5 */
    eTDAL_DISP_BLENDER_ID_6, /* Blender identifier 6 */
    eTDAL_DISP_BLENDER_ID_7  /* Blender identifier 7 */
...
typedef enum
{
	...
	eTDAL_DISP_LAYER_VIDEO_ID_4,  /* Layer video identifier 4 */
	eTDAL_DISP_LAYER_VIDEO_ID_5,  /* Layer video identifier 5 */
	eTDAL_DISP_LAYER_VIDEO_ID_6,  /* Layer video identifier 6 */
	eTDAL_DISP_LAYER_VIDEO_ID_7,  /* Layer video identifier 7 */
	/* ... */
	...
	eTDAL_DISP_LAYER_GFX_ID_4,    /* Layer graphics identifier 4 */
	/* ... */
...
typedef struct
{
    ...
    uint8_t     bBackgroundColor;               /* FALSE: no background colour supported on this blender.
...
typedef struct
{
    ...
    uint8_t    IsMultiRegionOnSameLineCapable; /* It is only relevant if NbRegionMax > 1.
                                               TRUE if the layer is able to display more than one region in a same line.
                                               FALSE otherwise. */
    uint8_t    IsInputWindowCapable;           /* TRUE if input windowing is supported by the layer. FALSE otherwise. */
    uint8_t    IsOutputWindowCapable;          /* TRUE if output windowing is supported by the layer. FALSE otherwise. */
    uint8_t    IsGlobalTransparencyCapable;    /* TRUE if the layer is able to perform a global transparency. */
    uint8_t    IsAspectRatioWindowConvCapable; /* TRUE if TDAL_DISP implementation is capable of resizing output window
...
/* Generic */
extern tTDAL_DISP_Error TDAL_DISP_Init (void);
extern tTDAL_DISP_Error TDAL_DISP_Term (void);
extern tTDAL_DISP_Error TDAL_DISP_CapabilityGet(tTDAL_DISP_Capability *pstCapability);
extern const char *     TDAL_DISP_APIRevisionGet(void );
extern const char *     TDAL_DISP_PlatformRevisionGet(void );
/* Blender */
extern tTDAL_DISP_Error TDAL_DISP_BlenderCapabilityGet(tTDAL_DISP_BlenderId Id,
                                                       tTDAL_DISP_BlenderCapability *pstCapability);
extern tTDAL_DISP_Error TDAL_DISP_BlenderBackgroundColorSet(tTDAL_DISP_BlenderId Id,
                                                            tTDAL_DISP_BlenderBackgroundColor *pstColor);
extern tTDAL_DISP_Error TDAL_DISP_BlenderDimensionsGet(tTDAL_DISP_BlenderId Id,
                                                       uint32_t *pWidth,
                                                       uint32_t *pHeight);
extern tTDAL_DISP_Error TDAL_DISP_BlenderDimensionsSet(tTDAL_DISP_BlenderId Id,
                                                       uint32_t Width,
                                                       uint32_t Height);
extern tTDAL_DISP_Error TDAL_DISP_BlenderDimensionsOutputSet(tTDAL_DISP_BlenderId Id,
                                                       	tTDAL_DISP_BlenderWindow *pstOutputWindow);
extern tTDAL_DISP_Error TDAL_DISP_BlenderDisable(tTDAL_DISP_BlenderId Id);
extern tTDAL_DISP_Error TDAL_DISP_BlenderEnable(tTDAL_DISP_BlenderId Id);
extern tTDAL_DISP_Error TDAL_DISP_BlenderZOrderPositionsGet(tTDAL_DISP_BlenderId Id,
                                                            tTDAL_DISP_BlenderZOrderPositions *pstZOrderList);
extern tTDAL_DISP_Error TDAL_DISP_BlenderZOrderListSet(tTDAL_DISP_BlenderId Id,
                                                       tTDAL_DISP_LayerId *pstZOrderList);

/* Layer */
extern tTDAL_DISP_Error TDAL_DISP_LayerCapabilityGet(tTDAL_DISP_LayerId Id,
                                                     tTDAL_DISP_LayerCapability *pstCapability);
extern tTDAL_DISP_Error TDAL_DISP_LayerGlobalTransparencySet(tTDAL_DISP_LayerId Id,
                                                             uint8_t Transparency);
extern tTDAL_DISP_Error TDAL_DISP_LayerHandleGet(tTDAL_DISP_LayerId Id,
                                                 void *pHandle);

extern tTDAL_DISP_Error TDAL_DISP_LayerHandleSet(tTDAL_DISP_LayerId Id,
                                                 void *pHandle);

extern tTDAL_DISP_Error TDAL_DISP_LayerIOWindowsAdjust(tTDAL_DISP_LayerId Id,
                                                       tTDAL_DISP_LayerWindow *pstInputWindow,
                                                       tTDAL_DISP_LayerWindow *pstOutputWindow,
                                                       uint8_t *pbAdjusted);
extern tTDAL_DISP_Error TDAL_DISP_LayerIOWindowsAspectRatioParamsSet(tTDAL_DISP_LayerId Id,
                                                                     tTDAL_DISP_LayerWindow *pstInputWindow,
                                                                     tTDAL_DISP_AspectRatio InputAspectRatio,
                                                                     uint32_t InputAFD,
                                                                     tTDAL_DISP_AspectRatioConv AspectRatioConv);
extern tTDAL_DISP_Error TDAL_DISP_LayerIOWindowsGet(tTDAL_DISP_LayerId Id,
                                                    tTDAL_DISP_LayerWindow *pstInputWindow,
                                                    tTDAL_DISP_LayerWindow *pstOutputWindow);
extern tTDAL_DISP_Error TDAL_DISP_LayerIOWindowsSet(tTDAL_DISP_LayerId layerId,
                                                    tTDAL_DISP_LayerWindow *pstInputWindow,
                                                    tTDAL_DISP_LayerWindow *pstOutputWindow);
extern tTDAL_DISP_Error TDAL_DISP_LayerDisable(tTDAL_DISP_LayerId Id);
extern tTDAL_DISP_Error TDAL_DISP_LayerEnable(tTDAL_DISP_LayerId Id);
extern tTDAL_DISP_Error TDAL_DISP_LayerLock(tTDAL_DISP_LayerId id);
extern tTDAL_DISP_Error TDAL_DISP_LayerRelease(tTDAL_DISP_LayerId id);
