...
tCliCommands gCliCommands[] =
{
    ...
    // TODO: Verify how stream analyzation functions are defined as possible command
...
// TODO: Verify how analyze function is defined as possible command
void DtvPlayer_Execute_Analyze(size_t argc, char** argv)
{
    static const char *optString = "h?";
    ...
    while (opt != -1)
    {
        ...
        switch (opt)
        {
            case 'h':
            case '?':
                DtvPlayer_Comm_Print("analyze # no params needed");
                return;
            default:
                break;
        }
