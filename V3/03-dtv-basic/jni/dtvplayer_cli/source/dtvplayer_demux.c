#include <stdio.h>
#include <string.h>

#include "tdal_common.h"
#include "tdal_dmd.h"
#include "tdal_dmx.h"

#include "dtvplayer_utils.h"
#include "dtvplayer_demux.h"


static const tTDAL_DMX_DemuxId DEMUX_ID = eTDAL_DMX_DEMUX0;

// Maximal programs inside stream
#define MAX_PROGRAM_COUNT 10

static tErrorCode DtvPlayer_Demux_StartPsiFiltering(tDmxRequest* request);
static tErrorCode DtvPlayer_Demux_StopPsiFiltering(tDmxRequest* request);

// Demux PSI table data callback fn declaration
static void DtvPlayer_Demux_DataCallbackFct ( tTDAL_DMX_ChannelId ChannelId,    /* channel ID */
                                              tTDAL_DMX_FilterId  FilterId,     /* filter ID */
                                              uint32_t            Filter_track, /* track */
                                              uint32_t            Length,       /* length of the buffer */
                                              uint8_t             *Buffer);


// PMT table requests containers
static tDmxRequest gPmtDmxRequest[MAX_PROGRAM_COUNT];
// PAT table request container
static tDmxRequest gPatDmxRequest;

/**
 * @brief THis function is used to initialize demux module
 * 
 * @return tErrorCode 
 */
tErrorCode DtvPlayer_Demux_Init()
{
    tTDAL_DMX_Error error;
    // TODO: Call DMX Module initialization in SDK
    // NOTE 1: Find TDAL_DMX_Init function in API and provide appropriate params
    // NOTE 2: Check for errors
     error = TDAL_DMX_Init();
    if (error != kTDAL_DMX_NO_ERROR) {
        LOGE("[%s:%d] TDAL_DMX_Init failed with error: %d", __FUNCTION__, __LINE__, error);

        return error;
    }

    return ePLAYER_ERROR_OK;
}

/**
 * @brief This function is used to start scanning for PSI tables.
 * Initially this function has to create PAT request to be able to recive PMT data.
 * Once PAT table is received it has to be parsed and request PMT tables
 * 
 * @return tErrorCode 
 */
tErrorCode DtvPlayer_Demux_AnalyzePsi()
{
    tErrorCode error;
    // Initialize PAT request
    gPatDmxRequest.pid = 0;
    gPatDmxRequest.tableId = TID_PAT;

    // TODO: Start PSI filgPatDmxRequesttering
    // NOTE 1: PSI Filtering always starts with filtering PAT table first
    // NOTE 2: After we filter PAT table we will get required information to parse other tables
    // NOTE 3: Once we receive PAT info we will be able to request info for PMT table
    error = DtvPlayer_Demux_StartPsiFiltering(&gPatDmxRequest);
    if (error != ePLAYER_ERROR_OK) {
        LOGE("[%s:%d] Failed to start PSI filtering for PAT. Error: %d", __FUNCTION__, __LINE__, error);
        return error;
    }
    
    return ePLAYER_ERROR_OK;
}

/**
 * @brief This function is used to deinitialize demux module
 * 
 * @return tErrorCode 
 */
tErrorCode DtvPlayer_Demux_Deinit()
{
    if(gPatDmxRequest.pid != -1) {
        DtvPlayer_Demux_StopPsiFiltering(&gPatDmxRequest);
    }
    TDAL_DMX_Terminate();
    return ePLAYER_ERROR_OK;
}

/**
 * @brief This fdmx_error = TDAL_DMX_Allocate_Channel(DEMUX_ID, &stType, &channelId);
    if (dmx_error != kTDAL_DMX_NO_ERROR) return ConvertTDALDMXErrorToTErrorCode(dmx_error);

    // Register channel callback
    dmx_error = TDAL_DMX_Register_Channel_Callback(channelId, DtvPlayer_Demux_DataCallbackFct);
    if (dmx_error != kTDAL_DMX_NO_ERROR) return ConvertTDALDMXErrorToTErrorCode(dmx_error);

    // Set channel PID
    dmx_error = TDAL_DMX_Set_Channel_PID(channelId, request->pid);
    if (dmx_error != kTDAL_DMX_NO_ERROR) return ConvertTDALDMXErrorToTErrorCode(dmx_error);

    // Allocate filter
    dmx_error = TDAL_DMX_Allocate_Filter(channelId, &filterId);
    if (dmx_error != kTDAL_DMX_NO_ERROR) return ConvertTDALDMXErrorToTErrorCode(dmx_error);

    // Set filter
    dmx_error = TDAL_DMX_Set_Filter(filterId, &filter, &mask, &mode, size, crcMode);
    if (dmx_error != kTDAL_DMX_NO_ERROR) return ConvertTDALDMXErrorToTErrorCode(dmx_error);

    // Enable channel
    dmx_error = TDAL_DMX_Control_Channel(channelId, eTDAL_DMX_CTRL_ENABLE);
    if (dmx_error != kTDAL_DMX_NO_ERROR) return ConvertTDALDMXErrorToTErrorCode(dmx_error);

    // Enable filter
    dmx_error = TDAL_DMX_Control_Filter(filterId, eTDAL_DMX_CTRL_ENABLE);
    if (dmx_error != kTDAL_DMX_NO_ERROR) return ConvertTDALDMXErrorToTErrorCode(dmx_error);

    // Update the request structure with allocated channelId and filterId
    request->channelId = channelId;
    request->filterId = filterId;

    return ePLAYER_ERROR_OK;rCode 
 */
static tErrorCode DtvPlayer_Demux_StartPsiFiltering(tDmxRequest* request)
{
    tTDAL_DMX_Error       dmx_error = kTDAL_DMX_NO_ERROR;
 
    uint8_t                 size = 1;
    uint8_t                 filter = request->tableId;
    uint8_t                 mask = 0xFF;
    uint8_t                 mode = 0xFF;
    uint32_t                filter_track = request->tableId;
    tTDAL_DMX_FilterCRCMode crcMode = eTDAL_DMX_CRC_IGNORE;

    tTDAL_DMX_ChannelType_t stType;
    stType.eChannelMemorySize = eTDAL_DMX_HIGHT_MEMORY_SIZE;
    stType.eChannelOutput = eTDAL_DMX_MEMORY;
    stType.eChannelStream = eTDAL_DMX_SECTION_STREAM;

    // TODO: Implement table filtering
    // TODO: We need to allocate channel and filter for requested table, create data notification mechanism 
    // TODO: and start channel and filtering
    // NOTE 1: Find TDAL_DMX_Allocate_Channel function in API and provide appropriate params
    // NOTE 1.1: Provide channel id from request structure!!!
    // NOTE 2: Find TDAL_DMX_Register_Channel_Callback function in API and provide appropriate params
    // NOTE 2.1: Use DtvPlayer_Demux_DataCallbackFct as data callback
    // NOTE 3: Find TDAL_DMX_Set_Channel_PID function in API and provide appropriate params
    // NOTE 4: Find TDAL_DMX_Allocate_Filter function in API and provide appropriate params
    // NOTE 4.1: Provide filter id from request structure!!!
    // NOTE 5: Find TDAL_DMX_Set_Filter function in API and provide appropriate params
    // NOTE 5.1: When setting filter use values already initialized on entry of this function
    // NOTE 6: Find TDAL_DMX_Control_Channel function in API and provide appropriate params
    // NOTE 7: Find TDAL_DMX_Control_Filter function in API and provide appropriate params
    // NOTE 8: Implement Error checks
    dmx_error = TDAL_DMX_Allocate_Channel(eTDAL_DMX_DEMUX0, &stType, &request->channelId);
    if (dmx_error != kTDAL_DMX_NO_ERROR) return dmx_error;

    // Register channel callback
    dmx_error = TDAL_DMX_Register_Channel_Callback(request->channelId, DtvPlayer_Demux_DataCallbackFct);
    if (dmx_error != kTDAL_DMX_NO_ERROR) return dmx_error;

    // Set channel PID
    dmx_error = TDAL_DMX_Set_Channel_PID(request->channelId, request->pid);
    if (dmx_error != kTDAL_DMX_NO_ERROR) return dmx_error;

    // Allocate filter
    dmx_error = TDAL_DMX_Allocate_Filter(request->channelId, &request->filterId);
    if (dmx_error != kTDAL_DMX_NO_ERROR) return dmx_error;

    // Set filter
    //dmx_error = TDAL_DMX_Set_Filter(request->channelId, request->filterId, &filter, size, &mask, &mode, crcMode);
    dmx_error = TDAL_DMX_Set_Filter(request->channelId, request->filterId, filter, size, &filter, &mask, &mode, crcMode);
    if (dmx_error != kTDAL_DMX_NO_ERROR) return dmx_error;

    // Enable channel
    dmx_error = TDAL_DMX_Control_Channel(request->channelId, eTDAL_DMX_CTRL_ENABLE);
    if (dmx_error != kTDAL_DMX_NO_ERROR) return dmx_error;

    // Enable filter
    dmx_error = TDAL_DMX_Control_Filter(request->filterId, eTDAL_DMX_CTRL_ENABLE);
    if (dmx_error != kTDAL_DMX_NO_ERROR) return dmx_error;

    return ePLAYER_ERROR_OK;
}

/**
 * @brief This function is used to stop and release resources from previously started PSI parsing.
 * 
 * @param request previously started request
 * @return tErrorCode 
 */
static tErrorCode DtvPlayer_Demux_StopPsiFiltering(tDmxRequest* request)
{
    tTDAL_DMX_Error dmx_error = kTDAL_DMX_NO_ERROR;

    // TODO: Implement stop mechanism for table filtering, we need to stop filter and channel, and release allocated resources
    // NOTE 1: Find TDAL_DMX_Control_Filter function in API and provide appropriate params to stop filtering (eTDAL_DMX_CTRL_DISABLE)
    // NOTE 2: Find TDAL_DMX_Control_Channel function in API and provide appropriate params to stop channel (eTDAL_DMX_CTRL_DISABLE)
    // NOTE 3: Find TDAL_DMX_Free_Filter function in API and provide appropriate params
    // NOTE 4: Find TDAL_DMX_Free_Channel function in API and provide appropriate params
    // NOTE 5: Implement Error checks

     // Disable filter
    dmx_error = TDAL_DMX_Control_Filter(request->filterId, eTDAL_DMX_CTRL_DISABLE);
    if (dmx_error != kTDAL_DMX_NO_ERROR) {
        LOGE("[%s:%d] Failed to disable filter. Error: %d", __FUNCTION__, __LINE__, dmx_error);
        // Convert and return the error code
        return dmx_error;
    }

    // Disable channel
    dmx_error = TDAL_DMX_Control_Channel(request->channelId, eTDAL_DMX_CTRL_DISABLE);
    if (dmx_error != kTDAL_DMX_NO_ERROR) {
        LOGE("[%s:%d] Failed to disable channel. Error: %d", __FUNCTION__, __LINE__, dmx_error);
        // Convert and return the error code
        return dmx_error;
    }

    // Free filter
    dmx_error = TDAL_DMX_Free_Filter(request->filterId);
    if (dmx_error != kTDAL_DMX_NO_ERROR) {
        LOGE("[%s:%d] Failed to free filter. Error: %d", __FUNCTION__, __LINE__, dmx_error);
        // Convert and return the error code
        return dmx_error;
    }

    // Free channel
    dmx_error = TDAL_DMX_Free_Channel(request->channelId);
    if (dmx_error != kTDAL_DMX_NO_ERROR) {
        LOGE("[%s:%d] Failed to free channel. Error: %d", __FUNCTION__, __LINE__, dmx_error);
        // Convert and return the error code
        return dmx_error;
    }

    memset(request, -1, sizeof(tDmxRequest));
    return ePLAYER_ERROR_OK;
}

/**
 * @brief This is demux callback function which is called by TDAL DMX module with table section data
 * 
 * @param ChannelId channel id for which request was created
 * @param FilterId filter id for which request was created
 * @param Filter_track table id for which request was created
 * @param Length buffer length
 * @param Buffer data
 */
static void DtvPlayer_Demux_DataCallbackFct ( tTDAL_DMX_ChannelId    ChannelId,
                                              tTDAL_DMX_FilterId      FilterId,
                                              uint32_t                Filter_track,
                                              uint32_t                Length,
                                              uint8_t                 *Buffer)
{
    LOGD("[%s:%d] callback called.", __FUNCTION__, __LINE__);
    LOGD("[%s:%d] ChannelId [%d], Filter_track [%d], length [%d].",
                __FUNCTION__, __LINE__, ChannelId, Filter_track, Length);
    
    if(Filter_track == TID_PAT)
    {
        tErrorCode player_error;
        printf("PAT Table found!");
        player_error = DtvPlayer_Demux_StopPsiFiltering(&gPatDmxRequest);
        if(player_error != ePLAYER_ERROR_OK)
        {
            LOGE("[%s:%d] DtvPlayer_Demux_StopPsiFiltering failed. Error: [%d]", __FUNCTION__, __LINE__, player_error);
        }
    } else if(Filter_track == TID_PMT) {
        printf("PMT Table found!");
        // Will be implemented
    }
}
