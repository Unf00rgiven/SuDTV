 ...
 *
 * FILE NAME: $URL: http://svnsrv/svn/iwedia-ext/dev/sources/chal/core/tdal/trunk/inc/tdal_hdmi.h $
 *            $Rev: 5012 $
 *            $Date: 2012-11-13 12:32:08 +0100 (Tue, 13 Nov 2012) $
 *
...
/*--------------------------------------*/
#define kTDAL_HDMI_NB_VIDEO_FORMAT_MAX  (64)
#define kTDAL_HDMI_NB_AUDIO_FORMAT_MAX  (32)
...
typedef enum
{
    ...
    eTDAL_HDMI_VIDEO_RES_720x480I,       /*   720x480 interlaced  */
    eTDAL_HDMI_VIDEO_RES_720x480P,       /*   720x480 progressive */
    eTDAL_HDMI_VIDEO_RES_720x576I,       /*   720x576 interlaced  */
    eTDAL_HDMI_VIDEO_RES_720x576P,       /*   720x480 progressive */
    eTDAL_HDMI_VIDEO_RES_1280x720P,      /*  1280x720 progressive */
    eTDAL_HDMI_VIDEO_RES_1920x1080P,     /* 1920x1080 progressive */
    eTDAL_HDMI_VIDEO_RES_1920x1080I,     /* 1920x1080 interlaced  */
    eTDAL_HDMI_VIDEO_RES_1920x1035I,     /* 1920x1035 progressive */
    eTDAL_HDMI_VIDEO_RES_1920x1035P,     /* 1920x1035 interlaced  */
    eTDAL_HDMI_VIDEO_RES_1280x1152I,     /* 1280x1152 progressive */
    eTDAL_HDMI_VIDEO_RES_3840x2160P,     /* 3840x2160 progressive */
    eTDAL_HDMI_VIDEO_RES_4096x2160P,     /* 4096x2160 progressive */
    eTDAL_HDMI_VIDEO_RES_1280x720P_3D,   /*  1280x720 progressive 3D */
    eTDAL_HDMI_VIDEO_RES_1920x1080P_3D,  /* 1920x1080 progressive 3D */
...
typedef struct
{
    ...
    uint8_t    EDIDParsingCapable; /* Indicates if parsing of E-DID data buffer is implemented in TDAL_HDMI */
...
typedef struct
{
    uint8_t     bIsDigitalSignal;        /* TRUE if sink device supports digital signal. FALSE Otherwise */
    uint8_t     bIsSetupExpected;        /* Only relevant if IsDigitalSignal is FALSE.
                                        It indicates if blank-to-black setup is available */
    uint8_t     bIsSepSyncSupported;     /* Only relevant if IsDigitalSignal is FALSE
                                        It indicates if the seperate sync is supported */
    uint8_t     bIsCompSyncSupported;    /* Only relevant if IsDigitalSignal is FALSE.
                                        It indicates if the composite sync is supported*/
    uint8_t     bIsSyncOnGreenSupported; /* Only relevant if IsDigitalSignal is FALSE
                                        It indicates if sync on green video is supported */
    uint8_t     bIsPulseRequired;        /* Only relevant if IsDigitalSignal is FALSE.
                                        It indicates if assertion of the Vsync is supported */
    uint8_t     bIsVesaDFPCompatible;    /* Only relevant if IsDigitalSignal is TRUE.
...
typedef struct
{
    uint8_t                       bIsStandby;              /* TRUE if the sink device standby is supported */
    uint8_t                       bIsSuspend;              /* TRUE if the sink device suspend is supported */
    uint8_t                       bIsActiveOff;            /* TRUE if the sink device active off is supported */
    uint8_t                       bIsRGBDefaultColorSpace; /* TRUE if RGB is the default colour space */
    uint8_t                       bIsPreferredTimingMode;  /* TRUE if preferred timing mode is indicated in 1st detailed timing block */
    uint8_t                       bIsDefaultGTFSupported;  /* TRUE if GTF (Generalized Timing Formula) is supported */
...
typedef struct
{
    ...
    uint8_t                         bIsManufacturerTiming; /* It indicates if the established timings were adjusted
                                                          and verified at the factory */
} tTDAL_HDMI_EDIDEstablishedTiming;

typedef struct
{
    uint8_t     bIsTimingUsed; /* TRUE means that this standard timing mode is used. FALSE it is not used */
...
typedef struct
{
    ...
    uint8_t                          bIsInterlaced;        /* TRUE if interlaced mode. FALSE otherwise */
    uint8_t                          bIsHorizPolaPositive; /* TRUE if horizontal polarity is positive. FALSE otherwise */
    uint8_t                          bIsVertPolaPositive;  /* TRUE if vertical polarity is positive. FALSE otherwise*/
...
typedef struct
{
    ...
    uint8_t                     bIsSecondGTFSupported;  /* TRUE if secondary GTF is supported */
...
typedef struct
{
    ...
    uint8_t                                 bDetailedTimingDesc2ndFill;
    tTDAL_HDMI_EDIDTimingDescription        stDetailedTimingDesc2nd;
    uint8_t                                 bMonitorDescFill;
...
{
    uint8_t                           bIsNative;       /* TRUE if video format indicated by VideoIdentCode
...
typedef struct
{
    uint8_t bIsFrontRLSupported;       /* TRUE if front right/left speakers is supported by sink device */
    uint8_t bIsFrontCenterSupported;   /* TRUE if front center speakers is supported by sink device */
    uint8_t bIsFrontRLCenterSupported; /* TRUE if front right/left center speakers is supported by sink device */
    uint8_t bIsLFESupported;           /* TRUE if LFE (Low Frequency effect) speaker is supported by sink device */
    uint8_t bIsRearRLSupported;        /* TRUE if rear right/left speakers is supported by sink device */
    uint8_t bIsRearCenterSupported;    /* TRUE if rear center speakers is supported by sink device */
    uint8_t bIsRearRLCenterSupported;  /* TRUE if rear right/left center speakers is supported by sink device */
} tTDAL_HDMI_EDIDCEA_SADB;

typedef struct
{
    uint8_t     bLatencyPresent;
    uint8_t     bInterlacedLatencyPresent;
    uint32_t uiVideoLatency;
    uint32_t uiAudioLatency;
    uint32_t uiInterlacedVideoLatency;
    uint32_t uiInterlacedAudioLatency;
    uint8_t     bDeepColor30BitsPerPixelSupported;
    uint8_t     bDeepColor36BitsPerPixelSupported;
    uint8_t     bDeepColor48BitsPerPixelSupported;
    uint8_t     bDeepColorYCbCr444Supported;
...
typedef struct
{
    uint8_t                    		bIsUnderscanSink;       /* TRUE if sink device supports underscan */
    uint8_t                    		bIsBasicAudioSupported; /* TRUE if sink device supports basic audio */
    uint8_t                    		bIsYCbCr444Supported;   /* TRUE if sink device supports YCbCr444 in addition to RGB */
    uint8_t                    		bIsYCbCr422Supported;   /* TRUE if sink device supports YCbCr422 in addition to RGB */
...
typedef struct
{
    ...
    uint8_t                      bIsEDIDExtensionUsed; /* TRUE if E-DID block extension is filled with data.
                                                         FALSE otherwise */
...
typedef struct
{
    uint8_t 						bhasActiveFormatInformation;
...
IMPORT tTDAL_HDMI_Error TDAL_HDMI_Start(tTDAL_HDMI_SourceId Id, tTDAL_HDMI_CallbackFct NotifyFct, uint8_t AVIDisplayFormatForced);
