                                              ...
                                              uint8_t                 *Buffer)
{
    ...
    if(Filter_track == TID_PAT)
    {
        ...
        //table.
        //gPatDmxRequest.pid = 0;
        //gPatDmxRequest.tableId = TID_PAT; table->programs[program_index]
        int program_index = 0;
        for(; program_index < table.program_size; program_index++) {
            gPmtDmxRequest[program_index].pid = table.programs[program_index].program_map_pid;
            gPmtDmxRequest[program_index].tableId = TID_PMT;
            DtvPlayer_Demux_StartPsiFiltering(&gPmtDmxRequest[program_index]);
        }
        
        // TODO: Create PMT requests based on data read in PAT table
        // NOTE 1: Pay attention that PAT cable contains different tables and we dont need to filter for these
        // NOTE 2: When creating request use gPmtDmxRequest
        ...
        // DONE: Parse PMT table and print result
        parser_error = DtvPlayer_ParserPMTParse(Buffer, &table);
        if(parser_error != PARSER_SUCCESS) {
            LOGE("[%s:%d] DtvPlayer_Demux_ParserPMTParse failed. Error: [%d]", __FUNCTION__, __LINE__, parser_error);
        }
        // NOTE 1: Find DtvPlayer_ParserPMTParse function in API and provide appropriate params
        DtvPlayer_ParserPMTDump(&table);
        // NOTE 2: Find DtvPlayer_ParserPMTDump function in API and provide appropriate params
        dmx_error = TDAL_DMX_Free_Buffer(Buffer);
        if(dmx_error != kTDAL_DMX_NO_ERROR) {
            LOGE("[%s:%d] DtvPlayer_Demux_TDAL_DMX_Free_Buffer failed. Error: [%d]", __FUNCTION__, __LINE__, parser_error);
        }
